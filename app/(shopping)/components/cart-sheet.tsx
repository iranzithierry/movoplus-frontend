...
export default function ShoppingCartSheet({ open, setOpen }: { open: boolean; setOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
  const { cartProducts, refreshCartedProducts, localStorageCartProducts } = useGlobal();
  ...
  const sortedCartProducts = [...cartProducts].sort((a, b) => {
    const localStorageOrder = localStorageCartProducts?.cartProducts.findIndex((p) => p.id === a.id) - localStorageCartProducts?.cartProducts.findIndex((p) => p.id === b.id);
        ...
        <div className="flow-root">
          <ul role="list" className="-my-6 divide-y divide-gray-200">
            {loading ? (
              ...
              sortedCartProducts && sortedCartProducts.map((product, idx) => <CartSheetItem key={idx} product={product} />)
            )}
          </ul>
        </div>
      </ScrollArea>
