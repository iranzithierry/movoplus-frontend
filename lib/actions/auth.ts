...
export async function signup(state: AuthFormState, formData: FormData): Promise<AuthFormState> {
  ...
  let wait = false;
  try {
    const user = await getApiClient().then((client) => client.users.usersCreate(validatedFields.data));
    if (!('tokens' in user)) {
...
export async function login(state: AuthFormState, formData: FormData): Promise<AuthFormState> {
  ...
  try {
    const user = await getApiClient().then((client) => client.auth.authTokenCreate(validatedFields.data));
    if (!('tokens' in user)) {
