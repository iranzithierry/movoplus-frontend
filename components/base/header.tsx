...
export default function Header({ setOpen }: { setOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  return (
    <header className="relative">
      <p className="flex h-10 items-center justify-center bg-indigo-600 px-4 text-sm font-medium text-white sm:px-6 lg:px-8">Get free delivery on orders over 40K RWF</p>
                    ...
                    {({ open }) => (
                      <>
                        <div className="relative flex">
                          <PopoverButton className={cn(open ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-700 hover:text-gray-800', 'relative z-10 -mb-px flex items-center border-b-2 pt-px text-sm font-medium transition-colors duration-200 ease-out')}>{category.name}</PopoverButton>
                        </div>
